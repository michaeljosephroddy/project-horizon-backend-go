...
var _ repository.IAnalyticsRepository = (*MockAnalyticsRepository)(nil)
...
func (m *MockAnalyticsRepository) StandardDeviationMood(userID int, startDate time.Time, endDate time.Time) (float64, error) {
...
func (m *MockAnalyticsRepository) MoodTagStats(userID int, startDate time.Time, endDate time.Time) ([]model.TagStat, error) {
	args := m.Called(userID, startDate, endDate)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func (m *MockAnalyticsRepository) StandardDeviationSleep(userID int, startDate time.Time, endDate time.Time) (float64, error) {
	args := m.Called(userID, startDate, endDate)
	return args.Get(0).(float64), args.Error(1)
}

func (m *MockAnalyticsRepository) SleepQualityTagStats(userID int, startDate time.Time, endDate time.Time) ([]model.TagStat, error) {
	args := m.Called(userID, startDate, endDate)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func (m *MockAnalyticsRepository) MedicationOverviewStats(userID int, startDate time.Time, endDate time.Time) (float64, error) {
	args := m.Called(userID, startDate, endDate)
	return args.Get(0).(float64), args.Error(1)
}
...
func TestAnalyzeMedication_Success(t *testing.T) {
	...
	mockRepo.On("MedicationOverviewStats", userID, startDate, endDate).Return(85.5, nil)
